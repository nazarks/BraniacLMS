...
class TestNewsPage(TestCase):
    ...
    def setUp(self):
        ...
        self.client_with_auth.post(path_auth, data={"username": "admin", "password": "admin@123"})
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        self.selenium.find_element(By.ID, "id_password").send_keys("admin@123")
        button_enter.click()
        # Wait for footer
    ...
    def test_next_page(self):
        ...
        self.selenium.get(path)
        page_1_news_title = (
            WebDriverWait(self.selenium, 5).until(EC.visibility_of_element_located((By.CLASS_NAME, "card-title"))).text
        )
        print("\nTrying move to next page ...")
        path_page_2 = "?page=2"
        button_page_2 = WebDriverWait(self.selenium, 5).until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, f'[href="{path_page_2}"]'))
        )
        button_page_2.click()
        page_2_news_title = (
            WebDriverWait(self.selenium, 5).until(EC.visibility_of_element_located((By.CLASS_NAME, "card-title"))).text
        ...
        try:
            ...
            print("Failed Moved to next page")
            raise
